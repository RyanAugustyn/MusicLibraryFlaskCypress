...
const EditSong = ({ handleClose, show, props }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    await axios
      .put(`http://127.0.0.1:5000/api/songs/${props.id}`, updatedSong)
      .then(alert("Song updated!"));
    handleClose();
      ...
      <section className="modal-main">
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            defaultValue={props.release_date}
          />
          <label>Running Time</label>
          <input
            name="running_time"
            onChange={(e) => setRunningTime(e.target.value)}
            defaultValue={props.running_time}
          />
          <button type="submit" data-test="update song button">
            Update Song
          </button>
        </form>
